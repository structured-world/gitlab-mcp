...
// Import tool arrays
import { filesReadOnlyToolsArray } from './tools-readonly';
import { filesWriteTools } from './tools';
import type { ToolDefinition } from '../../types';

// Create combined or read-only tools array based on environment
export const filesTools: ToolDefinition[] = isReadOnly
  ? filesReadOnlyToolsArray
  : [...filesReadOnlyToolsArray, ...filesWriteTools];

// Export read-only tool names for backward compatibility
export const filesReadOnlyTools = filesReadOnlyToolsArray.map((tool) => tool.name);
