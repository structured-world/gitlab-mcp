...
import { getFilteredJobTokenScopeTools, getJobTokenScopeReadOnlyToolNames } from './registry';
...
const jobTokenScopeToolsFromRegistry = getFilteredJobTokenScopeTools(isReadOnly);

// Convert enhanced tool definitions to regular tool definitions for backward compatibility
export const jobTokenScopeTools: ToolDefinition[] = jobTokenScopeToolsFromRegistry.map(
  (tool): ToolDefinition => ({
    name: tool.name,
...
export const jobTokenScopeReadOnlyTools = getJobTokenScopeReadOnlyToolNames();
