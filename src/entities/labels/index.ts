...
// Import tool arrays
import { labelsReadOnlyToolsArray } from './tools-readonly';
import { labelsWriteTools } from './tools';
import type { ToolDefinition } from '../../types';

// Create combined or read-only tools array based on environment
export const labelsTools: ToolDefinition[] = isReadOnly
  ? labelsReadOnlyToolsArray
  : [...labelsReadOnlyToolsArray, ...labelsWriteTools];

// Export read-only tool names for backward compatibility
export const labelsReadOnlyTools = labelsReadOnlyToolsArray.map((tool) => tool.name);
