...
import { getFilteredLabelsTools, getLabelsReadOnlyToolNames } from './registry';
...
const labelsToolsFromRegistry = getFilteredLabelsTools(isReadOnly);

// Convert enhanced tool definitions to regular tool definitions for backward compatibility
export const labelsTools: ToolDefinition[] = labelsToolsFromRegistry.map(
  (tool): ToolDefinition => ({
    name: tool.name,
...
export const labelsReadOnlyTools = getLabelsReadOnlyToolNames();
