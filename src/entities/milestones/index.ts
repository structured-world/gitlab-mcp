...
import { getFilteredMilestonesTools, getMilestonesReadOnlyToolNames } from './registry';
...
const milestonesToolsFromRegistry = getFilteredMilestonesTools(isReadOnly);

// Convert enhanced tool definitions to regular tool definitions for backward compatibility
export const milestoneTools: ToolDefinition[] = milestonesToolsFromRegistry.map(
  (tool): ToolDefinition => ({
    name: tool.name,
...
export const milestoneReadOnlyTools = getMilestonesReadOnlyToolNames();
