...
// Import tool arrays
import { mrsReadOnlyToolsArray } from './tools-readonly';
import { mrsWriteTools } from './tools';
import type { ToolDefinition } from '../../types';

// Create combined or read-only tools array based on environment
export const mrsTools: ToolDefinition[] = isReadOnly
  ? mrsReadOnlyToolsArray
  : [...mrsReadOnlyToolsArray, ...mrsWriteTools];

// Export read-only tool names for backward compatibility
export const mrsReadOnlyTools = mrsReadOnlyToolsArray.map((tool) => tool.name);
