...
import { getFilteredSnippetsTools, getSnippetsReadOnlyToolNames } from './registry';
...
const snippetsToolsFromRegistry = getFilteredSnippetsTools(isReadOnly);

// Convert enhanced tool definitions to regular tool definitions for backward compatibility
export const snippetsTools: ToolDefinition[] = snippetsToolsFromRegistry.map(
  (tool): ToolDefinition => ({
    name: tool.name,
...
export const snippetsReadOnlyTools = getSnippetsReadOnlyToolNames();
