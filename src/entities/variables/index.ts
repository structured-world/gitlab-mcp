...
import { getFilteredVariablesTools, getVariablesReadOnlyToolNames } from './registry';
...
const variablesToolsFromRegistry = getFilteredVariablesTools(isReadOnly);

// Convert enhanced tool definitions to regular tool definitions for backward compatibility
export const variablesTools: ToolDefinition[] = variablesToolsFromRegistry.map(
  (tool): ToolDefinition => ({
    name: tool.name,
...
export const variablesReadOnlyTools = getVariablesReadOnlyToolNames();
