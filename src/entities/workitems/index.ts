...
import { getFilteredWorkitemsTools, getWorkitemsReadOnlyToolNames } from './registry';
...
const workitemsToolsFromRegistry = getFilteredWorkitemsTools(isReadOnly);

// Convert enhanced tool definitions to regular tool definitions for backward compatibility
export const workitemsTools: ToolDefinition[] = workitemsToolsFromRegistry.map(
  (tool): ToolDefinition => ({
    name: tool.name,
...
export const workitemsReadOnlyTools = getWorkitemsReadOnlyToolNames();
