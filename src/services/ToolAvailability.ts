...
export class ToolAvailability {
  ...
  private static toolRequirements: Record<string, ToolRequirement> = {
    ...
    run_container_scan: {
      minVersion: "10.8",
      requiredTier: "ultimate",
      notes: "Container scanning",
    },
    run_secret_detection: {
      minVersion: "13.1",
      requiredTier: "ultimate",
      notes: "Secret detection",
    },
  ...
  public static isToolAvailable(toolName: string, action?: string): boolean {
    ...
    try {
      ...
      const version = parseVersion(instanceInfo.version);
      if (version < parseVersion(requirement.minVersion)) {
        return false;
      }
  ...
  public static getToolsByMinVersion(minVersion: string): string[] {
    ...
    return Object.entries(this.toolRequirements)
      .filter(([, requirement]) => parseVersion(requirement.minVersion) >= minVer)
      .map(([toolName]) => toolName);
  }
}
