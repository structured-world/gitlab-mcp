...
describe('projectIdentifier', () => {
  ...
  describe('validateProjectIdentifier', () => {
    ...
    it('should reject invalid inputs', () => {
      expect(validateProjectIdentifier('')).toBe(
        'Project identifier is required and must be a string',
      );
      expect(validateProjectIdentifier('   ')).toBe('Project identifier cannot be empty');
      expect(validateProjectIdentifier(null as any)).toBe(
        'Project identifier is required and must be a string',
      );
      expect(validateProjectIdentifier(undefined as any)).toBe(
        'Project identifier is required and must be a string',
      );
      expect(validateProjectIdentifier(123 as any)).toBe(
        'Project identifier is required and must be a string',
      );
    });
    ...
    it('should reject invalid characters', () => {
      ...
      expect(result).toBe(
        'Invalid project identifier format. Use numeric ID or namespace/project path.',
      );
    });
